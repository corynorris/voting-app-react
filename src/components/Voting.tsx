...
export function Voting() {
  ...
  const remaining = useCountdown(state?.matchStartedAt ?? 0, state?.timerSeconds ?? 0);
  ...
  useEffect(() => {
    ...
    if (prevScoresRef.current && scoresKey !== prevScoresRef.current && scoresKey !== "0-0") {
      playSound("vote");
    }
  ...
  useEffect(() => {
    if (remaining > 0 && remaining <= 5 && Math.abs(remaining - Math.round(remaining)) < 0.15) {
      playSound("tick");
    }
  ...
  const mainView = match.decided || hasVotes ? (
    <ResultsView
      match={match}
      round={currentRound}
  ...
  return (
    <div className="lobby">
      ...
      <div className="lobby-form">
        ...
        <button
          className="start-btn"
          disabled={!name.trim() || entryText.split("\n").filter(Boolean).length < 2}
          onClick={handleStart}
        >
          Start Tournament
        </button>
...
function TimerBar({ remaining }: { remaining: number }) {
  ...
  return (
    <div className={`timer-bar${isUrgent ? " urgent" : ""}`}>
      <div className="timer-bar-fill" style={{ width: `${(remaining / 30) * 100}%` }} />
    ...
    <div className="voting">
      ...
      <div className="vote-pair">
        <button
          ...
          onClick={() => onVote(entryA)}
        >
          <h1>{entryA}</h1>
        </button>
        <button
          className="vote-btn pair-b"
          onClick={() => onVote(entryB)}
        >
          <h1>{entryB}</h1>
        </button>
      </div>
